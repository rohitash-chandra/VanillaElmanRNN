...
class NeuralNetwork: public virtual TrainingExamples { 
 ...
 int StringSize;
  ...
  int BackPropogation(TrainingExamples TraineeSamples, double LearningRate,Sizes Layersize,char* Savefile,char* TestFile, int sampleSize, int columnSize, int outputSize, ofstream &out1,  ofstream &out2);
...
{    int end = Layersize.size() - 1;
    ...
    for(int pattern = 0; pattern< temp; pattern++){
    for(int output = 0; output < Layersize[end]; output++) {
      Error =   (TraineeSamples.Sample[pattern].OutputValues[output]  -Output[pattern][output]) * MAEscalingfactor ;
...
return accuracy;
 ...
 int NeuralNetwork::BackPropogation(TrainingExamples TraineeSamples, double LearningRate,Sizes Layersize, char * Savefile,char* TestFile, int sampleSize, int columnSize, int outputSize, ofstream &out1,  ofstream &out2)
{
   ...
   double  Train  =0;
      if (epoch%100==0){
      SumErrorSquared = SumSquaredError(TraineeSamples,TraineeSamples.SampleSize ,Layersize);

         double mae =               MAE(TraineeSamples,TraineeSamples.SampleSize ,Layersize); 
        cout<<SumErrorSquared<< "     "<<mae <<" " <<epoch<<endl;

       out1<<SumErrorSquared<< "     "<<mae <<" " <<" "<<Train<<epoch<<endl;
}
...
int Cycles;
       ...
       void   Procedure(bool bp,   double h, ofstream &out1, ofstream &out2, ofstream &out3 );
};



void    Simulation:: Procedure(bool bp,   double h, ofstream &out1, ofstream &out2, ofstream &out3  )
{
	 ...
	 clock_t start = clock();
		   ...
		   int gene = 1;
		    ...
		    epoch = network.BackPropogation(Samples,learningRate,layersize,file,trainfile, trainsize,input,output ,  out1, out2);//  train the network

	       out2<<"Train"<<endl; 
             Train =  network.TestTrainingData( layersize,file,trainfile, trainsize,input,output,out2);
              TrainNMSE =       network.NMSError();
              out2<<"Test"<<endl;
             Test =  network.TestTrainingData( layersize,file,testfile, testsize,input,output,out2);
  ...
  out3<<"  "<<h<<"  "<<TotalEval<<"  RMSE:  "<<Train<<"  "<<Test<<" NMSE:  "<<TrainNMSE<<" "<<TestNMSE<<endl;
